...
Of course, if you're using the config object to store datas, it'll already be created. [Read more on the config Storage in our API reference](https://yeoman.github.io/generator/Storage.html).
...
Also, [helpers assertions methods](https://yeoman.github.io/generator/assert.html) are now exposed on the `yeoman.assert` namespace.
