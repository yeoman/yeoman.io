...
Of course, if you're using the config object to store datas, it'll already be created. [Read more on the config Storage in our API reference](http://yeoman.io/generator/generator/Storage.html).
...
Also, [helpers assertions methods](http://yeoman.io/generator/generator/assert.html) are now exposed on the `yeoman.assert` namespace.
